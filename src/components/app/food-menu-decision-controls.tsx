...
import { decideFoodMenuAction } from "@/app/actions/food-menu";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import type { ActionState } from "@/lib/types";

type FoodMenuDecisionControlsProps = {
  menuId: string;
};

export function FoodMenuDecisionControls({ menuId }: FoodMenuDecisionControlsProps) {
  ...
  function review(decision: "approve" | "reject") {
    startTransition(async () => {
      const result = await decideFoodMenuAction(menuId, decision, note);
      setState(result);
    });
  }
